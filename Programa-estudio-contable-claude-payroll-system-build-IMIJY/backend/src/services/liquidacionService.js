...
async function calcularLiquidacionMensual(empleadoId, anio, mes, opciones = {}) {
  ...
  let otrosDescuentos = opciones.otrosDescuentos ?? [];
  let otrosHaberes    = opciones.otrosHaberes    ?? [];
  ...
  // Conceptos asignados individualmente al empleado vigentes en este período
  const periodoInicio = new Date(anio, mes - 1, 1);
  const periodoFin    = new Date(anio, mes, 0); // último día del mes
  const conceptosAsignados = await prisma.conceptoEmpleado.findMany({
    where: {
      empleadoId,
      activo: true,
      vigenciaDesde: { lte: periodoFin },
      OR: [{ vigenciaHasta: null }, { vigenciaHasta: { gte: periodoInicio } }],
    },
    include: { concepto: true },
  });

  for (const ca of conceptosAsignados) {
    const c = ca.concepto;
    const importe = Number(ca.valor ?? c.importe ?? 0);
    if (importe === 0) continue;
    if (c.naturaleza === 'HABER') {
      otrosHaberes = [...otrosHaberes, {
        descripcion: c.nombre,
        importe,
        remunerativo: c.tipo === 'REMUNERATIVO',
        tipo: c.tipo,
        codigo: c.codigo,
        conceptoId: c.id,
      }];
    } else if (c.naturaleza === 'DESCUENTO') {
  ...
  // Factor part-time: si el empleado trabaja menos horas que el convenio
  const convenioRef = empleado.convenio || empleado.empresa.convenio;
  const convenioHoras = Number(convenioRef?.horasSemanales ?? 40);
  const empHoras = empleado.horasSemanales ? Number(empleado.horasSemanales) : convenioHoras;
  const factorPartTime = convenioHoras > 0 ? empHoras / convenioHoras : 1;
  ...
  // El básico se ajusta por factor part-time (ej: 25hs/40hs = 0.625)
  const basico = calcs.redondear(Number(empleado.basicoMensual) * factorPartTime);

  // Acuerdos paritarios vigentes para el convenio del empleado
  const convenioId = empleado.convenioId || empleado.empresa.convenioId;
  if (convenioId) {
    const acuerdos = await prisma.acuerdoSalarial.findMany({
      ...
      orderBy: [{ vigenciaDesde: 'asc' }, { cuota: 'asc' }],
    });

    for (const a of acuerdos) {
      let importe = 0;
      if (a.monto) {
        importe = calcs.redondear(Number(a.monto) * factorPartTime);
      } else if (a.porcentaje) {
        importe = calcs.redondear(basico * Number(a.porcentaje) / 100);
      }
      if (importe === 0) continue;
      const cuotaStr = a.cuota && a.totalCuotas ? ` (cuota ${a.cuota}/${a.totalCuotas})` : '';
      otrosHaberes = [...otrosHaberes, {
        descripcion: `${a.descripcion}${cuotaStr}`,
        importe,
        remunerativo: a.tipo === 'REMUNERATIVO',
        tipo: a.tipo,
        codigo: `ACP`,
      }];
    }
