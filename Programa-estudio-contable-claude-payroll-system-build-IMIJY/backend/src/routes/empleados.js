...
// GET /api/empleados/:id/conceptos
router.get('/:id/conceptos', auth, [param('id').isUUID(), validate], async (req, res, next) => {
  try {
    const registros = await prisma.conceptoEmpleado.findMany({
      where: { empleadoId: req.params.id },
      include: { concepto: { select: { id: true, codigo: true, nombre: true, tipo: true, naturaleza: true, unidad: true, remunerativo: true } } },
...
// POST /api/empleados/:id/conceptos
router.post('/:id/conceptos', auth, [
  param('id').isUUID(),
  ...
  try {
    const { conceptoId, valor, vigenciaDesde, vigenciaHasta, observaciones } = req.body;
    const registro = await prisma.conceptoEmpleado.create({
      data: {
...
// PUT /api/empleados/conceptos/:ceId
router.put('/conceptos/:ceId', auth, [param('ceId').isUUID(), validate], async (req, res, next) => {
  try {
    const { valor, vigenciaDesde, vigenciaHasta, activo, observaciones } = req.body;
    const registro = await prisma.conceptoEmpleado.update({
      where: { id: req.params.ceId },
      data: {
        ...(valor !== undefined && { valor: valor != null ? valor : null }),
...
// DELETE /api/empleados/conceptos/:ceId
router.delete('/conceptos/:ceId', auth, [param('ceId').isUUID(), validate], async (req, res, next) => {
  try {
    await prisma.conceptoEmpleado.delete({ where: { id: req.params.ceId } });
    res.json({ ok: true });
  } catch (err) { next(err); }
});
